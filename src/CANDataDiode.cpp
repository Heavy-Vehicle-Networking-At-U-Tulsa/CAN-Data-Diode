...
//#define spi_readwrite SPI_transfer
...
INT8U spi_readwrite(INT8U data)
{
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE){
    for (INT8U i = 0;i<8;i++){
      ...
      if ((data & 0x80) == 0x80){
		PORTB |= 0b00000001; //Set the bit on PB0
		data_in <<=1;
		data_in =  ((PORTB >> 1)&0x01)|data_in;
      }
	  else {PORTB &= 0b1111110;} //clear the bit on PB0
      PORTB |= 0b00000100; //Clock out the data on PB2
      data <<= 1;
      data_in <<=1;
      data_in =  ((PORTB >> 1)&0x01)|data_in; //Bitwise OR with PB1 to build the input
